...
Uzytkownik PlikZUzytkownikami::pobierzDaneUzytkownika(string daneJednegoUzytkownikaOddzielonePionowymiKreskami)
{
    ...
    int dlugoscLancuchaZnakow = daneJednegoUzytkownikaOddzielonePionowymiKreskami.length();

    for (int pozycjaZnaku = 0; pozycjaZnaku < dlugoscLancuchaZnakow; pozycjaZnaku++)
    {
...
void PlikZUzytkownikami::zapiszWszystkichUzytkownikowDoPliku(vector <Uzytkownik> &uzytkownicy)
{
    ...
    if (plikTekstowy.good() == true)
    {
        for (int k = 0; k < iloscUzytkownikow; ++k)
        {
            liniaZDanymiUzytkownika = zamienDaneUzytkownikaNaLinieZDanymiOddzielonaPionowymiKreskami(uzytkownicy[k]);
